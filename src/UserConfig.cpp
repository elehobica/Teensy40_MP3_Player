...
int UserConfig::getValue(int category_idx, int item_idx)
{
...
/*
int UserConfig::getSelIdx(int category_idx, int item_idx)
{
    if (category_idx >= sz_category) { return 0; }
    if (item_idx >= category[category_idx].size) { return 0; }
    config_item_t *item = &category[category_idx].items[item_idx];
...
void UserConfig::setSelIdx(int category_idx, int item_idx, int sel_idx)
{
...
void UserConfig::scanSelIdx(void (*yield_func)(int adrs, int *val), bool do_hook_func)
{
    int adrs = 0;
...
const char *UserConfig::getStr(int idx)
{
...
bool UserConfig::isSelection()
{
    return (level == 2);
}

bool UserConfig::selIdxMatched(int idx)
{
